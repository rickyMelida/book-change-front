...
const bookContainer = document.querySelector('#favorite-book');
const itemWishList = document.querySelector('#favorites-item img');

document.addEventListener('DOMContentLoaded', ()=>{
    removeAllAssetsExcept(itemWishList);
    for(let i=0; i<=3;i++) {
        createCardsBooks(myData.outstandingBooks[i], bookContainer);
    }
});
...
function createCardsBooks(data, container) {
    ...
    imgBookmark.setAttribute("src", "../img/bookmark_active.png");
...
function removeAllAssetsExcept(item) {
    ...
    item.setAttribute('src', '../img/wish_list_active.svg');
}